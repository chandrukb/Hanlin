...
- (void)textFieldDidEndEditing:(UITextField *)textField{
    ...
    if(validateOnResign)
        [(HNTextField *)textField validate];
}

- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string{
    [(HNTextField *)textField dismissPopup];
    if(validateOnCharacterChanged)
        [(HNTextField *)textField performSelector:@selector(validate) withObject:nil afterDelay:0.1];
    else
        [(HNTextField *)textField setRightView:nil];
...
-(void)addConfirmValidationTo:(HNTextField *)txtConfirm withMsg:(NSString *)msg{
...
-(BOOL)validate{
    ...
    for (int i=0; i<[arrRegx count]; i++) {
        ...
        if([dic objectForKey:@"confirm"]){
            HNTextField *txtConfirm=[dic objectForKey:@"confirm"];
